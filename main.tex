...
\[
    t \approx \frac{\bar{X}_1 - \bar{X}_2}{\sigma_{\text{anat}} \sqrt{\frac{4}{n}}}
\]
...
\begin{equation*}
    \begin{split}
        ...
        = \mathrm{Var}\!\left(\frac{\bar{X}_1 - \bar{X}_2}{\sigma_{\text{anat}}}\right) \frac{n}{4} \\[4pt]
                   & = \frac{4}{n} \nu_{\text{nav}}^2 \frac{n}{4}                                   \\[4pt]
                   & = \nu_{\text{nav}}^2.
    \end{split}
\end{equation*}
...
\paragraph{ANCOVA} is a general linear model that includes covariates to control for potential
confounding factors. The model can be expressed as:
\[
    Y = X\beta + \varepsilon
\]
where $Y$ is the response variable (e.g., brain measurements), $X$ is the
design matrix including group indicators and covariates, $\beta$ is the vector
of regression coefficients, and $\varepsilon$ is the error term. To assess the
impact of numerical variability on the estimated coefficients $\hat{\beta}$, we
consider the variance of $\hat{\beta}$, which is given by:
\[
    \mathrm{Var}(\hat{\beta}) = \sigma_{\text{num}}^2 (X^TX)^{-1}
\]
Assuming that the numerical variability $\sigma_{\text{num}}^2$ is small
compared to the variability explained by the model, we can approximate the
variance of the estimated coefficients as:
\[
    \mathrm{Var}(\hat{\beta}) \approx \nu_{\text{nav}}^2 (X^TX)^{-1}
\]
This relationship indicates that the uncertainty in the estimated coefficients
due to numerical variability is directly proportional to the \navr, scaled by
the inverse of the design matrix's information content.
