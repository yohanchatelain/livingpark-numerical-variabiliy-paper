...
{   \begin{tikzpicture}
    ...
    {   \ifthenelse{\c = 1}
        {}
        {   \fill[left color=\lowcolo,right color=\x] ({(\c-2)*\piecewidth},0) rectangle ({(\c-1)*\piecewidth},#2);
        }
        \xdef\lowcolo{\x}
    }
    ...
    { \draw ({(\x-#4)/(#5-#4)*#1},0) -- ++ (0,-0.1) node[below=0.05,font=\tiny] {\pgfmathprintnumber{\x}};
    }
    \end{tikzpicture}
}
...
\begin{abstract}
    ...
    instability of FreeSurfer, a ubiquitous tool, using structural MRI data from
    the Parkinson's Progression Markers Initiative. By introducing controlled
    perturbations with Monte Carlo Arithmetic, we assessed the variability of
    cortical and subcortical measurements. We found that numerical variability
    in FreeSurfer is substantial, with key structural metrics often precise to
    only a single significant digit. This computational noise was up to half of
    the anatomical variation between Parkinson's disease patients and healthy
    controls. Consequently, the statistical significance of group differences
    and correlations with clinical severity fluctuated dramatically across
    identical analyses. We design a tool to quantify the Numerical-Anatomical
    Variability Ratio (\navr), which relates computational uncertainty to
    biological variation as well as practical formula to determine the
    population sample size required to achieve a desired variability on Cohen's
    d effect size. Our results demonstrate that computational errors can produce
    unreliable findings in clinical neuroimaging such as correlation and group
    analyses. Addressing such numerical instability is essential for developing
    robust biomarkers for neurological disorders like Parkinson's disease.
\end{abstract}
...
appendix. We selected 103 healthy controls (HC) and 121 Parkinson's disease (PD)
patients from the Parkinson's Progression Markers Initiative (PPMI) database.

We tested the statistical significance of group differences and correlations
with clinical measures (UPDRS scores) across the 26 MCA repetitions. We reported
the fraction of statistically significant tests ($p < 0.05$) for each metric and
region. Statistical significance proportions varied substantially for cortical
thickness (Figure~\ref{fig:significance_correlation_thickness}) and subcortical
volumes (Figure~\ref{fig:significance_correlation_subcortical_volume}). Ratios
near 0.5 indicated maximal uncertainty, while values approaching 0 or 1
suggested consistent results across computational variations.
...
can remove values $|\sigma_d|\leq 2/\sqrt{N} \text{\navr}$ as they
...
\begin{figure}[h]
    \centering
    \begin{tikzpicture}
        \node[anchor=base,inner sep=0] at (0,0) {\includegraphics[width=\textwidth]{figures/NAVR_map/NAVR_subcortical_volume_all.png}};
        \node[anchor=base,inner sep=0] at (0,-1) {\colorbar{12}{0.35}{blue!50!black,blue,teal,cyan,green,yellow,orange,red,red!50!black}{0}{0.5}{0.1}};
    \end{tikzpicture}
...
\begin{figure}[h]
    \centering
    % Jet: ['rgb(0,0,131)', 'rgb(0,60,170)', 'rgb(5,255,255)', 'rgb(255,255,0)', 'rgb(250,0,0)', 'rgb(128,0,0)']
    \begin{tikzpicture}
        \node[anchor=base,inner sep=0] at (0,0) {\includegraphics[width=\textwidth]{figures/NAVR_map/NAVR_thickness_all.png}};
        \node[anchor=base,inner sep=0] at (0,-1) {\colorbar{12}{0.35}{blue!50!black,blue,teal,cyan,green,yellow,orange,red,red!50!black}{0}{0.5}{0.1}};
    \end{tikzpicture}
    \caption{ Numerical-Anatomical Variability Ratio (\navr) for cortical
...
\begin{figure}[h]
    ...
    \begin{minipage}[b]{\textwidth}
        ...
        \begin{minipage}[c]{0.95\textwidth}
            \begin{subfigure}[c]{0.48\textwidth}
                \includegraphics[width=\textwidth]{figures/cohen_d_map/enigma/depression_thickness_adult.png}
                \label{fig:enigma_depression_thickness_unthresholded}
            \end{subfigure}
            \hfill
            \begin{subfigure}[c]{0.48\textwidth}
                \includegraphics[width=\textwidth]{figures/cohen_d_map/enigma/depression_thickness_adult_thresholded.png}
    ...
    \caption{ENIGMA cortical thickness Cohen's d maps showing unthresholded
        ...
        \navr framework (right) for different disorders. Black
        regions indicate areas where Cohen's d values fall below the numerical
        variability threshold, demonstrating regions where reported effect sizes may
        be unreliable due to computational uncertainty.\label{fig:navr_enigma_thickness}}
\end{figure}
...
cortical measurements showing limited precision (1-2 significant digits) that
...
demonstrates how numerical variability can compromise reproducibility.
Effect size distributions showed substantial spread around standard
...
This study demonstrates significant numerical
...
and anatomical variation, revealing that numerical instability approaches half of
the anatomical variability in certain brain regions.

The theoretical relationship between \navr and Cohen's d uncertainty provides a
framework for predicting statistical reliability based on computational
precision. The relationship with sample size suggests that larger cohorts can
mitigate numerical variability effects, but sample sizes must be sufficiently
large to ensure reliable effect size estimates. This is particularly relevant
for studies with small sample sizes, where numerical variability can
significantly distort results. Our cohort is homogeneous in age, which minimizes
anatomical variability, a cohort using a broader age range would likely show
higher anatomical variability, hence reducing the impact of numerical
variability on effect size estimation. Moreover, the \navr values have been
obtained using FreeSurfer 7.3.1, which is known to have numerical precision
issues. Future versions of FreeSurfer may improve numerical stability,
potentially altering the \navr values and their relationship with effect size
uncertainty. Other neuroimaging software packages may exhibit different
numerical precision characteristics, and the \navr framework can be adapted to
assess their reliability. A more comprehensive evaluation of multiple
neuroimaging tools would provide a broader understanding of numerical precision
issues in the field, and hence extending the \navr framework scope.

The previous results thresholded with the \navr framework
demonstrate that many reported effect sizes in neuroimaging studies may be unreliable,
although no statistical significant results were thresholded. We found a correlation
between the Cohen's d values being thresholded and their significance, indicating that
our thresholding method is effective in identifying unreliable effect sizes.
This highlights the importance of considering numerical precision when interpreting
neuroimaging results, particularly in studies with small sample sizes or subtle
disease-related changes, which is not the case for ENIGMA studies, which
typically have large sample sizes (many thousands of participants).

The fuzzy-libm library used in this study provides a practical solution for
assessing numerical precision in neuroimaging software and has demonstrated its
effectiveness in several applications. However, it is important to note that
other sources of instabilities may exist in neuroimaging software, such as
algorithmic choices, numerical approximations, and data handling practices.
Future work should explore these aspects to provide a more comprehensive
understanding of numerical precision in neuroimaging analyses.
...
\begin{figure}[h]
    ...
    % Jet: ['rgb(0,0,131)', 'rgb(0,60,170)', 'rgb(5,255,255)', 'rgb(255,255,0)', 'rgb(250,0,0)', 'rgb(128,0,0)']
    \begin{tikzpicture}
        \node[anchor=base,inner sep=0] at (0,0) {\includegraphics[width=\textwidth]{figures/NAVR_map/NAVR_area_all.png}};
        \node[anchor=base,inner sep=0] at (0,-1) {\colorbar{12}{0.35}{blue!50!black,blue,teal,cyan,green,yellow,orange,red,red!50!black}{0}{0.5}{0.1}};   \end{tikzpicture}
    \caption{ Numerical-Anatomical Variability Ratio (\navr) for cortical
        surface area across regions and groups. Higher \navr values indicate
        greater computational uncertainty relative to biological
        variation. The color scale indicates the \navr value, with warmer colors
        indicating higher \navr values.\label{fig:navr_thickness}}
\end{figure}
...
\begin{figure}[h]
    ...
    % Jet: ['rgb(0,0,131)', 'rgb(0,60,170)', 'rgb(5,255,255)', 'rgb(255,255,0)', 'rgb(250,0,0)', 'rgb(128,0,0)']
    \begin{tikzpicture}
        \node[anchor=base,inner sep=0] at (0,0) {\includegraphics[width=\textwidth]{figures/NAVR_map/NAVR_volume_all.png}};
        \node[anchor=base,inner sep=0] at (0,-1) {\colorbar{12}{0.35}{blue!50!black,blue,teal,cyan,green,yellow,orange,red,red!50!black}{0}{0.5}{0.1}};   \end{tikzpicture}
    \caption{ Numerical-Anatomical Variability Ratio (\navr) for cortical
        volume across regions and groups. Higher \navr values indicate
        greater computational uncertainty relative to biological
        variation. The color scale indicates the \navr value, with warmer colors
        indicating higher \navr values.\label{fig:navr_thickness}}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{ENIGMA cortical area Cohen's d maps showing unthresholded effect
        ...
        framework (right) for different disorders. Black regions indicate areas
        where Cohen's d values fall below the numerical variability threshold,
        demonstrating regions where reported effect sizes may be unreliable due to
