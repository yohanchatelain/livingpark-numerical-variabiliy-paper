...
on conclusions drawn from MRI analyses of Parkinson's disease. We focused on (1) volumetric group
differences between PD subjects and Healthy Controls (HC), and (2) partial
correlations between regional volumes and motor evaluation scores (MDS-Unified
Parkinson's Disease Rating Scale (MDS-UPDRS) part 3). For both, we conducted a
cross-sectional analysis at baseline and a longitudinal analysis across two time
points.
...
disorders. The final dataset included  118 PD participants without MCI (PD-non-MCI) and
90 HC (Table~\ref{tab:cohort_stat}).
...
mathematical expectations. MCA was applied via dynamic library linking using
fuzzy libmath~\cite{salari2021accurate}. We repeated the perturbed analyses,
yielded 26 usable runs, to estimate numerical variability.
...
than indicating a methodological artifact. The spread of the r coefficients is larger
...
longitudinal studies. An analogous analysis for cortical thickness leads to similar
...
Although the importance of numerical variability is established, its practical
impact on individual studies remains difficult to quantify. Stochastic
arithmetic like Monte Carlo Arithmetic (MCA) can measure this variability but
are computationally expensive, making them hard for routine use with complex
neuroimaging pipelines. While new architectures~\cite{elarar2025ms2a} may
democratize stochastic-rounding in the coming years, MCA remains unsuitable for
routine use at study scale. This motivates the need for a fast, analytical
framework to estimate numerical uncertainty without re-running an entire
pipeline. Our approach builds on the assumption that numerical variability is a
property of the computational pipeline rather than the underlying anatomical
population specificity~\cite{bhagwat2021understanding,vila2024impact,chatelain2024numerical,sokolowski2024impact}.
This enables us to develop a generalizable statistical model for widespread
application.

To make this framework interpretable and comparable across datasets, we
introduce the Numerical-Anatomical Variability Ratio (\navr):
$$\nu_{\mathrm{nav}} = \frac{\sigma_{\mathrm{num}}}{\sigma_{\mathrm{anat}}}$$
which expresses the relative magnitude of numerical variability
($\sigma_{\mathrm{num}}$; Eq.~\eqref{eq:sigma_num}) to anatomical variability
($\sigma_{\mathrm{anat}}$; Eq.~\eqref{eq:sigma_anat}). \navr quantifies
the intrinsic numerical instability of a neuroimaging pipeline, allowing
comparison across regions and studies.

We then derived closed-form, first-order approximations for how numerical variability
perturbs four common statistical tests used in structural
MRI: Cohen's $d$, two-sample $t$-test, partial correlation, and ANCOVA. For each
of these tests, we obtained: (1) an approximation of the standard deviation of
the metric induced by numerical variability and (2) the corresponding
uncertainty in the $p$-value (Table~\ref{tab:stat_uncertainty}). These results
follow from propagating the numerical variability through the estimators using
the delta-method (derivations in Section~\ref{sec:theoretical_derivations}).

This analytical framework provides a practical link between the pipeline's
numerical instability (\navr), the study's sample size ($n$), and the
resulting uncertainty in effect sizes and $p$-values. Since the formulas
include the summary statistics of the study, it enables researchers to
audit the potential impact of numerical variability on existing
studies without requiring expensive recomputation or access to the original data.
...
Across cortical and subcortical regions, we found numerical variability
accounted for up to 37-40\% of the observed anatomical variance in baseline
analyses (Figures~\ref{fig:navr_subcortical},~\ref{fig:navr_thickness}).
Longitudinal analyses exhibited substantially higher \navr values with up to
XX-XX\%. At baseline, mean \navr values reached 0.21 for cortical thickness,
0.15 for subcortical volumes, and XX and XX for longitudinal studies, indicating
that numerical imprecision constitutes a portion of the anatomical signal.
Higher \navr values in longitudinal analyses suggest that tracking changes over
time is more susceptible to numerical noise, likely due to catastrophic
cancellation when computing differences between timepoints. Catastrophic
cancellation occurs when subtracting two nearly equal numbers, leading to a
significant loss of precision by promoting rounding errors. These values have
direct implications: for a typical \navr of 0.2, achieving a negligible
numerical uncertainty on Cohen's d effect size ($\sigma_d \leq 0.01$) would
require a sample size of at least 1,500 participants
...
To facilitate the widespread adoption of this framework, we provide an
interactive, public web tool available at
\href{https://yohanchatelain.github.io/brain\_render/}{yohanchatelain.github.io/brain\_render}.
\TG[done]{you could add a screenshot of the tool} allowing rapid reproducibility
audits and retrospective assessments of published findings (Appendix
Figure~\ref{fig:brain_render_tool}). Furthermore, we observed that regions
failing standard quality control (QC) checks consistently exhibited higher
\navr, suggesting the metric can also serve as a useful proxy for data quality.
By making numerical-variability evaluation accessible, our framework helps to
improve transparency, support peer review, and foster more reliable statistical
...
To assess the broader implications of numerical variability, we applied \navr to
re-evaluate influential findings from X articles, which has
