...
variability ($\sigma_{\mathrm{num}}$; Eq.\ref{eq:sigma_num}) and the
between-subject anatomical variability ($\sigma_{\mathrm{anat}}$;
...
\begin{table}[t]
    ...
    \begin{tabular}{lll}
        ...
        Partial correlation & $\sigma_r \geq \nu_{\mathrm{nav}}\sqrt{\frac{(1-r^{2})^{3}}{n-1}}$ & $\sigma_{p} \approx 2f_{t,df}(|t|)\sqrt{\frac{df}{n-1}}\nu_{\mathrm{nav}}$  \\
    ...
    \caption{First-order numerical uncertainty of common statistical tests under
        Monte Carlo Arithmetic perturbations. $f_{t,df}$ and $f_\mathcal{F}(F;1,df_2)$ denote the probability density functions of the Student's
        $t$-distribution with $df$ degrees of freedom and the $\mathcal{F}$-distribution with
        $(1, df_2)$ degrees of freedom, respectively. The $p$-value approximation for the partial correlation uses $t=r(df/(1-r^2))^{1/2}$.}
