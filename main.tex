...
% - $E[\bar X_i] = \mu_i$.
% - $Var[\bar X_i] = (2\sigma^2_{\text{num}})/n$.
% - $E[s_p] = \sigma^2_{anat}$
% - $V[s_p] = 2\sigma^2_{anat} / (n-1)$.
% - $V[d] = V[(\bar X_1 - \bar X_2) / s_p]$.
% - By delta method $V[d] ~ (Var[\bar X_1 - \bar X_2])/E[s_p]^2 + ((E[\bar X_1 - \bar X_2]^2 . V[s_p]) / E[s_p]^4)$.
% - So $V[d] = 2/N . \sigma^2_{\text{num}} / \sigma^2_{\text{anat}}.$
% - $V[d] = 2/N . \nu^2_{nav}.$
% - $\sigma_d = 2/\sqrt{N} . \nu^2_{nav}$
...
standard deviation of Cohen's $d$ ($\sigma_d$). Cohen's $d$ is defined as:
...
and \(s_2\) are the sampled standard deviations, and \(n_1\) and \(n_2\) are the sample
sizes of the two groups. The numerator represents the difference in means, while
the denominator is the pooled standard deviation with a degree of freedom $n_{df} =
