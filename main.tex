...
impact of numerical variability. The key assumptions~\label{eq:assumptions_stats} are: (1) Null Hypothesis
...
standard deviation of the true biological values: $s_p \approx \sigma_{\text{anat}}$.
We use the following notation for the sample means, sample variances, and sample correlation for a sample size $n$:
\[
    \bar X=\tfrac1n\sum_i X_i,
    s_X^2=\tfrac1{n-1}\sum_i (X_i-\bar X)^2,
    s_{XY}=\tfrac1{n-1}\sum_i (X_i-\bar X)(X_i-\bar X).
\]
...
\section*{Conditional variance across repetitions for Cohen's $d$ (two-sample, pooled SD)}

\paragraph{Setup.}
Two independent groups $G_1$ and $G_2$ with sizes $n_1,n_2$ ($df=n_1+n_2-2$).
Across subjects, the data $\{x_i\}$ vary; across Monte-Carlo repetitions $r$, only $X$ is perturbed:
\[
    X_i^{(r)}=x_i+e_i^{(r)},\qquad \mathbb E[e_i^{(r)}]=0,\ \ \operatorname{Var}(e_i^{(r)})=\sigma_{\mathrm{num}}^2.
\]
Group statistics (Bessel):
\[
    \bar X_g=\frac{1}{n_g}\sum_{i\in G_g}X_i,\qquad
    s_g^2=\frac{1}{n_g-1}\sum_{i\in G_g}(X_i-\bar X_g)^2,\qquad g=1,2.
\]
Pooled SD $s_p=\sqrt{\frac{(n_1-1)s_1^2+(n_2-1)s_2^2}{df}}$, mean difference $\Delta=\bar X_1-\bar X_2$,
Cohen's $d=\Delta/s_p$.

\paragraph{Target quantity (conditional variance).}
\[
    \boxed{\quad
    \mathrm{Var}_{\mathrm{rep}}[d]\ :=\ \mathrm{Var}\!\left(d(X^{(r)})\ \big|\ \{x_i\}\right).
    \quad}
\]
Delta method about the baseline $X_0=x$ gives
\[
...
For $i\in G_1$, $\partial\bar X_1/\partial X_i=1/n_1$, $\partial\bar X_2/\partial X_i=0$, hence $\partial\Delta/\partial X_i=1/n_1$.
For $i\in G_2$, $\partial\Delta/\partial X_i=-1/n_2$.
Within a group of size $m$,
\(
\frac{\partial s}{\partial X_i}=\frac{X_i-\bar X}{(m-1)s}.
\)
Therefore
\[
    \frac{\partial s_p}{\partial X_i}
    =\begin{cases}
        \dfrac{X_i-\bar X_1}{df\,s_p}, & i\in G_1, \\[6pt]
        \dfrac{X_i-\bar X_2}{df\,s_p}, & i\in G_2.
    \end{cases}
\]
Quotient rule for $d=\Delta/s_p$ yields
\[
    \boxed{
        \frac{\partial d}{\partial X_i}
        =\begin{cases}
            \dfrac{1}{n_1 s_p}\ -\ \dfrac{\Delta}{df\,s_p^3}\,(X_i-\bar X_1),  & i\in G_1, \\[8pt]
            -\dfrac{1}{n_2 s_p}\ -\ \dfrac{\Delta}{df\,s_p^3}\,(X_i-\bar X_2), & i\in G_2.
        \end{cases}
    }
\]

\paragraph{Summation of squares.}
Use $\sum_{i\in G_g}(X_i-\bar X_g)=0$ and $\sum_{i\in G_g}(X_i-\bar X_g)^2=(n_g-1)s_g^2$.
Then
\[
    \sum_{i\in G_1}\!\left(\frac{\partial d}{\partial X_i}\right)^2
    = \frac{1}{n_1 s_p^2} + \frac{\Delta^2}{df^{2}s_p^{6}}\,(n_1-1)s_1^2,\qquad
    \sum_{i\in G_2}\!\left(\frac{\partial d}{\partial X_i}\right)^2
    = \frac{1}{n_2 s_p^2} + \frac{\Delta^2}{df^{2}s_p^{6}}\,(n_2-1)s_2^2.
\]
Since $(n_1-1)s_1^2+(n_2-1)s_2^2=df\cdot s_p^2$,
\[
    \sum_{i=1}^n\!\left(\frac{\partial d}{\partial X_i}\right)^2
    =\frac{1}{s_p^2}\!\left(\frac{1}{n_1}+\frac{1}{n_2}\right)
    +\frac{\Delta^2}{df\,s_p^4}.
\]
Noting $d=\Delta/s_p$,
\[
    \boxed{\quad
...
\[
    \boxed{\quad
    \mathrm{Var}_{\mathrm{rep}}[d]\ \approx\ \sigma_{\mathrm{num}}^2\ \frac{1}{s_p^2}\!
...
\[
    \mathrm{Var}_{\mathrm{rep}}[d]\ \approx\ \nabla_X d^\top\,\Sigma_{XX}\,\nabla_X d,
\]
with the elementwise $\partial d/\partial X_i$ given above. If $\Sigma_{XX}$ is diagonal with entries $\sigma_i^2$ (heteroskedastic),
\(
\mathrm{Var}_{\mathrm{rep}}[d]\approx \sum_{i=1}^n \sigma_i^2\left(\partial d/\partial X_i\right)^2.
\)

\paragraph{Remark (Hedges' $g$).}
If $g=J(df)\,d$ is reported instead, then
\(
\mathrm{Var}_{\mathrm{rep}}[g]\approx J(df)^2\,\mathrm{Var}_{\mathrm{rep}}[d]
\)
since $J(df)$ does not depend on $X$.
