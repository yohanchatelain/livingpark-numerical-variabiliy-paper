...
\TG{Rewrite the remainder of this paragraph with the following outline:
...
For both cortical and subcortical regions, we found substantial
discrepancies in statistical outcomes across the 26 numerically perturbed
analyses with $\simeq 20\%$ of tests yielding inconsistent results in average ($p<0.05$).

For subcortical volumes, statistical significance fluctuated notably across MCA
repetitions (Figure~\ref{fig:significance_correlation_subcortical_volume}). For
group comparisons alone, six subcortical regions ($21\%$ of tests) alternated between
significant and non-significant outcomes, illustrating how numerical instability
can directly affect clinical interpretation. For partial correlation, seven
subcortical regions ($25\%$ of tests) presented inconsistent results across numerical states.
Overall, 27\% of all statistical comparisons, including both group differences
and clinical correlations, yielded inconsistent results across numerical states.

For cortical thickness, statistical significance fluctuated across MCA
repetitions (Figure~\ref{fig:significance_correlation_thickness}). For group
comparisons alone, 14 cortical regions ($10\%$) alternated between significant and
non-significant outcomes, illustrating how numerical instability can directly
affect clinical interpretation. For partial correlation, 34 cortical regions
($25\%$) presented inconsistent results across numerical states. Overall, 19\% of all
statistical comparisons, including both group differences and clinical
correlations, yielded inconsistent results across numerical states.

Statistical significance between Parkinson's disease patients and controls for
subcortical volumes and cortical thickness fluctuated notably across MCA
replicates (Figures~\ref{fig:significance_correlation_subcortical_volume}
and~\ref{fig:significance_correlation_thickness}) \TG[done]{correlations are not
    discussed.}. Eight (out of 14; $57\%$) subcortical regions and 40 (out of 68;
$59\%$) cortical regions alternated between significant and non-significant
outcomes ($p < 0.05$),  illustrating how numerical instability directly affects
clinical interpretation. Overall, $20\%$ of the comparisons yielded inconsistent
results across numerical states, reaching $28\%$ when including all cortical
(thickness, area and volume) and subcortical measurements
(see Appendix Table~\ref{tab:fluctuating_regions}), suggesting that possible biomarker
relationships might emerge or vanish purely due to numerical noise. This
variability mirrors multi-team analytical differences seen in previous
...
F-statistics from ANCOVA analyses for subcortical volumes
(Figure~\ref{fig:statstest_coefficients_distribution}) and cortical thickness
(Appendix Figure~\ref{fig:consistency_thickness_coefficients}), we observe that the
unperturbed IEEE-754 results (red markers) consistently lie within the range
sampled by MCA. This confirms that the observed variability supports the
validity of the instrumentation \TG[done]{not really, the variability could be
    larger than the real one, reformulate to say that it ``supports the validity of
    the instrumentation''} rather than methodological artifacts. We can also notice
that the spread of the r coefficients and F-statistics is higher for
longitudinal analyses than for baseline analyses \TG{it's quite apparent for r
    coefficients but not so much for F-statistics. A statistical test would help.},
indicating that numerical variability has a more pronounced impact on
longitudinal studies. \TG{reviewers will wonder about a similar graph for
    cortical regions}
...
\TG{Instead of re-stating the importance of numerical variability in this
    ...
    need for a statistical correction.} Quantifying the influence of numerical variability is essential for
...
assess their relative impact, we define the Numerical-Anatomical Variability
Ratio (NAVR) as the ratio between numerical uncertainty
...
\]
\TG{I think I would rather present the $\sigma_d$ result first, then explain
...
with a typical NAVR of 0.2, suppressing numerical contributions \TG{not sure what this means, reformulate} below 0.01 in
...
web tool\footnote{\url{https://yohanchatelain.github.io/brain_render/}} \TG{you could add a screenshot of the tool},
...
statistical outcomes \TG{in Parkinson's disease analyses}, leading to the appearance or disappearance of clinically
...
This suggests that in smaller exploratory \TG{why exploratory?} studies, numerical instability may
undermine the reliability of reported effects. \TG{see previous comments: it
    would be nice to include analyses for some of such smaller studies}
...
is inherently non-associative \TG{what does this mean? Additions of
    floating-point numbers is not associative? If so, this is out of context} and
sensitive to compiler behavior, hardware architecture, and thread scheduling. As
a result, neuroimaging pipelines—though deterministic in design—can produce
divergent results across computational environments. Previous analyses of
FSL~\cite{mirhakimi2025numerical} and ANTs~\YC{cite Mathieu} indicate that such instability
is not unique to FreeSurfer, but likely pervades the field. SPM however seems to
be less impacted by numerical variability \TG{cite Niusha}. \TG{I think you'd
    need to refer to DL methods here, including FS8.} \TG{start a new paragraph
    here, on the generalizability across data cohorts. You could include a mention
    that numerical variability didn't differ between PD and HC, and possibly link to
    a supplementary figure to support the claim.} Moreover, our PD cohort was
relatively homogeneous in age and phenotype, potentially reducing anatomical
variance and inflating NAVR values. This highlights the need to \TG{I would
    moderate this statement a little bit. I think it would be interesting the
    measure the NAV in different pipelines and datasets, but I don't think there's a
    very strong need for that.} apply \TG{apply or measure?} NAVR across diverse
datasets, software packages, and disease contexts to build a comprehensive
understanding of computational reliability.

Crucially, this instability is not confined to low-level rounding operations \TG{instead what?}.
...
yield distinct model outcomes \TG{see previous comment on DL, I think we should refer to Ines' work here}. In neuroimaging, such instability means that
even identical inputs can lead to divergent interpretations, raising
...
\TG{I would merge this paragraph with the previous one, and highlight specific
    next steps regarding numerical variability in neuroimaging.}
...
This relationship emerges from error propagation analysis. \TG{The remainder of
this paragraph is quite informal, could you tidy up the equations to start from
the definition of Cohen's d and clearly derive its standard deviation? You should also clarify the assumptions that are made.} The
difference in group means has variance $\text{Var}(\bar{X}_1 - \bar{X}_2) =
