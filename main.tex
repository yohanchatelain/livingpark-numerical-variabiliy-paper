...
FreeSurfer 7.3.1 on the estimation of structural MRI measures and their
relationships with clinical outcomes in Parkinson's disease. To achieve this
goal, the study is divided into two main components: a cross-sectional analysis
and a longitudinal analysis.

\subsubsection{Cross-sectional Analysis}

The primary objective of the cross-sectional analysis is to measure the impact
of numerical variability in FreeSurfer version 7.3.1 on the PPMI (Parkinson's
Progression Markers Initiative) cohort. This will involve comparing the
estimation of structural MRI measures, including cortical and subcortical
volumes, cortical thickness, and surface area. The goal is to assess the
stability of these key metrics and determine how numerical variability may
affect their reliability in clinical research.

\subsubsection{Longitudinal Analysis}

In the longitudinal analysis, the focus shifts to evaluating the influence of
numerical variability on clinical research outcomes within the PD group.
Specifically, this analysis aims to (1) investigate how numerical variability
affects the detection of group differences in subcortical volumes and cortical
thickness between healthy controls (HC) and Parkinson's disease (PD) patients
over time. (2) Assess the extent to which numerical instability impacts
observed correlations between disease severity, as measured by clinical
outcomes (e.g., UPDRS scores), and brain structure measures in PD patients,
both at baseline and longitudinally.

By addressing these objectives, the study aims to provide valuable insights
into the reliability of FreeSurfer 7.3.1 for Parkinson's disease research.
Furthermore, it seeks to offer recommendations on how to mitigate the effects
of numerical variability in clinical neuroimaging studies, ultimately enhancing
the reproducibility and accuracy of MRI-derived measures in PD research.
...
\subsubsection{Significant digits formula}\label{sec:sig_digits}
...
Table~\ref{tab:cohort_stat}.\todo[inline]{The table must be before or after
    exclusion?} The study adhered to the Declaration of Helsinki and received an
...
\paragraph{Vertex-Wise Analyses}

To investigate the relationships between cortical thickness and clinical
measures over time, we conducted vertex-wise analyses. These analyses focused
on three key aspects:

\begin{itemize}
    \item The correlation between \textbf{UPDRS scores} and \textbf{baseline cortical
              thickness}.
    \item The correlation between the \textbf{rate of change} in UPDRS scores and the
          \textbf{rate of change} in cortical thickness.
    \item Group differences between \textbf{healthy controls (HC)} and
          \textbf{PD-non-MCI} in both \textbf{baseline cortical thickness} and its
          \textbf{longitudinal change}.
\end{itemize}

To assess statistical significance, we employed both \textbf{Z-tests} and
\textbf{permutation tests}. Z-tests evaluate whether a given measure
significantly deviates from the population mean under a normality assumption,
while permutation tests provide a non-parametric alternative that does not rely
on distributional assumptions. The results of these analyses are summarized in
the following sections.

\paragraph{Inconsistency of Statistically Significant Regions Across MCA Repetitions}
Due to the inherent variability introduced by Monte Carlo Arithmetic (MCA), the
significance of cortical regions fluctuated across repetitions. Specifically:
\begin{itemize}
    \item Only \textbf{two regions} were found to be statistically significant in
          \textbf{two repetitions out of 26}, suggesting a lack of statistical power.
    \item The surface sizes of significant regions varied considerably, with examples
          such as \textbf{1005.95 ± 39.34 mm²} and \textbf{996.83 ± 42.91 mm²}.
\end{itemize}

\subsection{Z-Tests Identified More Significant Regions Than Permutation Tests}
When comparing the two statistical approaches:
\begin{itemize}
    \item \textbf{7 regions} were found significant using the \textbf{Z-test}, whereas only \textbf{3 regions} were found significant using the \textbf{permutation test}.
    \item However, significant regions \textbf{were not consistent} across MCA
          repetitions, reinforcing concerns regarding replicability.
\end{itemize}

\subsection{Group Analysis}

\subsubsection{Z-Test Results}
The following table summarizes significant regions identified in the
\textbf{group analysis} using a \textbf{Z-test} (\(\alpha = 0.05\)). Each
region was statistically significant in only \textbf{one MCA repetition},
highlighting a \textbf{lack of replicability}.
...
No clusters were found to be statistically significant using the
\textbf{permutation test} (1000 permutations).

\subsection{Correlation Analysis}

\subsubsection{Z-Test Results}
The following table presents significant regions identified in the
\textbf{correlation analysis} using a \textbf{Z-test} (\(\alpha = 0.05\)).
Notably, only \textbf{two regions} were found to be significant in \textbf{two
    different MCA repetitions}, with variability in their estimated \textbf{size}
and \textbf{MNI coordinates}.
...
\begin{table}[h]
    ...
    \begin{tabular}{lllllll}
        ...
        \midrule
        \textbf{Baseline}   &                        &                &                &                &                 &                    \\
        L postcentral       & 1006.20 ± 39.27        & -37.0          & -33.8/-34      & 62.5/61.7      & -4.7429/-3.7576 & 2/25               \\
...
\subsubsection{Permutation Test Results}
The following regions were identified using a \textbf{permutation test} (1000
permutations) for the correlation analysis. Each region was significant in only
\textbf{one MCA repetition}, further demonstrating a \textbf{lack of
    replicability}.
