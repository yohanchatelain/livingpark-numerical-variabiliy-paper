...
\newcommand{\TG}[1]{\textcolor{blue}{\textsc{From Tristan:} #1}}
...
\TG{Rewrite using the following structure:
\begin{itemize}
\item The reliability of structural MRI measures critically depends on the estimation of sources of analytical variability.
\item In particular when effect sizes are moderate, such as in Parkinson's disease.
\item Among these sources, numerical variability has been shown to have a measurable impact on analyses across multiple pipeline tools.
\item Numerical variability arises from rounding and truncation errors associated with the use of limited-precision numerical formats. 
\item Due to the complexity of MRI analyses, and in particular their reliance on high-dimensional optimization, such errors propagate and sometimes lead to measurable differences.
...
\section{Impact of numerical variability on MRI measures of Parkinson's disease}
...
perturbations using Monte Carlo Arithmetic (MCA) \TG{add ref to Parker and Ali's
fuzzy libmath}, a technique to perturb floating-point operations with random,
zero-mean offsets that preserve mathematical expectations \TG{operations?}.
Repeatedly executing an analysis pipeline with MCA enables us to estimate the
variability of results that could arise from different computational
environments.
...
We evaluated the impact of numerical variability on MRI measures of Parkinson's
disease using \TG{x} subjects and \TG{y} healthy controls with T1 MRI
acquisitions available in the Parkinson's Progression Markers Initiative (PPMI)
\TG{add ref}. \TG{Rewrite the remainder of this paragraph to explain the type of
analyses that was done on the data, and why these analyses were selected.}
...
instead, we are genuinely interested in PD}, as structural MRI-derived
...
each time with a different random state. \TG{The remainder of this paragraph
refers to supplementary data, which doesn't read well as we're expecting to read
the key (main) results first. It should be reworked.} Testing for PD progression
partial correlations and group differences between PD patients and healthy
controls, 27\% of statistical tests are non-reproducible for subcortical volumes
and 19\% for cortical thickness (Extended Data
...
(Extended Data Fig.~\ref{fig:navr_subcortical} and~\ref{fig:navr_thickness}).
...
performed group comparisons and partial \TG{why partial?} correlations between
regional cortical thickness and subcortical volumes with clinical measures of
Parkinson's disease (UPDRS scores). \TG{Rewrite the remainder of this paragraph
with the following outline: (1) key message: "for both cortical and subcortical
regions, substantial discrepancies..., (2) focus on subcortical volumes
specifically, discussing the different analyses, (3) focus on cortical volumes,
(4) conclusion and reference to NARPS.} Statistical significance between
Parkinson's disease patients and controls for subcortical volumes fluctuated
notably across MCA replicates
(Figure~\ref{fig:significance_correlation_subcortical_volume}) \TG{correlations
are not discussed.}. Eight subcortical regions alternated between significant
and non-significant outcomes ($p < 0.05$), illustrating how numerical
instability directly affects clinical interpretation. Overall, 27\% of the
comparisons yielded inconsistent results across numerical states. Similarly,
statistical tests for cortical thickness varied substantially across numerical
states (Fig.~\ref{fig:significance_correlation_thickness}) with 19\% of the
comparisons yielding inconsistent results, suggesting that possible biomarker
...
sampled by MCA. This confirms that the observed variability represents realistic
computational scenarios \TG{not really, the variability could be larger than the
real one, reformulate to say that it ``supports the validity of the instrumentation''} rather than methodological artifacts. We
can also notice that the spread of the r coefficients and F-statistics is higher
for longitudinal analyses than for baseline analyses \TG{it's quite apparent for r coefficients but not so much for F-statistics. A statistical test would help.}, indicating that numerical
variability has a more pronounced impact on longitudinal studies. \TG{reviewers will wonder about a similar graph for cortical regions}
...
FSL~\YC{cite Niusha} and ANTs~\YC{cite Mathieu} indicate that such instability
