...
For both group analyses and correlation analyses, statistical outcomes varied
...
and~\ref{fig:significance_correlation_thickness}): $20\%$ of comparisons
(66/328) changed significance at the $p<0.05$ threshold. In subcortical volumes
(14 regions; Figure~\ref{fig:significance_correlation_subcortical_volume}),
$21\%$ of group comparisons and $25\%$ of partial correlations were inconsistent
across MCA repetitions, and $57\%$ exhibited at least one flip
in significance. Cortical thickness showed a similar pattern (68 regions;
...
Table~\ref{tab:fluctuating_regions}). These fluctuations indicate that candidate
biomarker relationships may appear or vanish due to numerical noise, echoing
multi-analyst variability reported in NARPS~\cite{botvinik2020variability}. \TG{The numbers in this paragraph are hard to follow as the $\%$
 refer to different totals. Could this be simplified to one main percentage per figure?}
...
longitudinal studies than on cross-sectional ones. An analogous analysis for cortical thickness leads to similar
...
is highly compute-intensive and impractical to perform routinely. In this
section, we introduce a practical analytical model of numerical variability in
MRI analyses, assuming that this variability is an inherent property of the
computational pipeline and generalizes across
populations.
% ~\cite{bhagwat2021understanding,vila2024impact,chatelain2024numerical,sokolowski2024impact}.
% \TG{explain what claims are supported by these references} 

More specifically, we derived closed-form approximations of numerical
uncertainty for common statistics and their associated p-values, which can be
readily incorporated into MRI analyses (Table~\ref{tab:stat_uncertainty}).
These formulas rely on the Numerical-Anatomical Variability Ratio
(\navr) that represents the relative magnitude of numerical variability
($\sigma_{\mathrm{num}}$; Eq.~\eqref{eq:sigma_num}) to population variability
...
rely only on the study's summary statistics, they enable researchers to
...
section (Figure~\ref{fig:navr}). \TG{Revise the following text when Figure 4 is final}
