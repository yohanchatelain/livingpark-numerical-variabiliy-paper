...
Structural images were processed using FreeSurfer 7.3.1 instrumented with Fuzzy-libm, a modified mathematical library that injects stochastic perturbations into floating-point operations to probe numerical stability (Section~\ref{sec:numerical_variability_assessment}). To estimate numerical variability, each scan was processed 34 times. We filtered the resulting outputs to exclude both technical failures \TG[done]{defined failed: failed due to technical reasons or failed QC?} (e.g., incomplete execution and FreeSurfer failures) and quality control (QC) failures. 

\TG[done]{you should mention QC too}
For QC, we visually inspected nine representative slices (3 along each axis) per run to identify gross artifacts, such as missing brain tissue, blurring, or biologically implausible segmentation. Table~\ref{tab:qc} details the exclusion rates. To ensure statistical consistency across the cohort, we randomly subsampled the remaining runs to retain exactly 26 valid realizations per participant.

\subsection{Numerical Variability Assessment}
\label{sec:numerical_variability_assessment}

To quantify the numerical instability inherent in the FreeSurfer pipeline, we
