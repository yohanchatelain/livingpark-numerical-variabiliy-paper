...
biologically implausible segmentation. Supplementary Table~\supptabref{tab:qc}{S4} details the
exclusion rates. To ensure statistical consistency across the cohort, we
randomly subsampled the remaining runs to retain exactly 26 valid realisations
per participant.
...
\subsubsection{Relationship between \npvr~and downstream statistical test uncertainty}
...
Proofs of partial derivations of sample statistics\footnote{mean, variance, standard-deviation, covariance, Pearson correlation coefficient and correlation identities.} used in the following sections are provided in the Supplementary Note~\suppnoteref{appendix:partial_derivatives_proofs}{S1}.
...
\]
and the derivatives of the correlations with respect to $x_i$ are
(see Supplementary Eq.~\suppequref{eq:pearson_derivative}{S5})
\[
...
\subsection{Probability of significance flips induced by numerical uncertainty}
...
\begin{figure}[h]
    ...
    \caption{\textbf{Modeling inference stability using Beta distributions.}
        ...
        $\alpha$ represents the probability of a false positive finding (\FP).}
