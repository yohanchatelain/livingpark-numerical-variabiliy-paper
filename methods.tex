...
PPMI cohort using FreeSurfer instrumented with stochastic numerical noise to
isolate run-to-run variability. Analytically, we derived closed-form
...
\begin{table}[h!]
    ...
    \caption{\textbf{Participant characteristics.} Values represent mean $\pm$
        ...
        subset corresponds to participants with available follow-up MRI and disease
...
\subsection{Numerical variability assessment\label{sec:numerical_variability_assessment}}
...
\subsubsection{Relationship between \npvr~and downstream statistical test uncertainty\label{sec:theoretical_derivations}}
...
$\sum\beta_i^2 = (n-1)(1 - b^{2})$, $\sum\alpha_i\beta_i = (n-1)(c - ab)$
(see Supplementary Eq.~\suppequref{eq:correlation_identities}{S6}), and that
$(1-b^{2})(1-c^{2})-{(a-bc)}^{2} = (1-a^{2})(1-b^{2})-{(c-ab)}^{2}$, we sum over all
$i$ to obtain:
\begin{align*}
    \sum_{i=1}^{n}
    {\left(\frac{\partial R}{\partial x_i}\right)}^{2}
    ...
    \left[
        ...
        \right].
\end{align*}
Substituting back into Eq.~\eqref{eq:delta-partial} with $s_x^2 \approx \sigma_{\mathrm{pop}}^2$ gives:
\begin{equation}
    \Var_{\mathrm{num}}[R]
    \approx
    \frac{\nu_{\mathrm{npv}}^{2}}{(n-1)}
...
\end{equation}
Since $b$ is rarely reported in practice, we further simplify
this expression by deriving a lower bound on $\Var_{\mathrm{num}}[R]$.
Since $0 \leq b^{2} \leq 1$, then $1/(1 - b^{2}) \geq 1$. Therefore:
\begin{equation}
...
\subsection{Probability of significance flips induced by numerical uncertainty\label{sec:false-positives-modeling}}
