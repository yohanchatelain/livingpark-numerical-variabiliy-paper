...
\TG[done]{include a summary of your methods here.}
\TG[done]{Overall the methods are quite brief, you should add more details so that
...
to exclude both technical failures \TG[done]{defined failed: failed due to
    technical reasons or failed QC?} (e.g., incomplete execution and FreeSurfer
failures) and quality control (QC) failures.

\TG[done]{you should mention QC too} For QC, we visually inspected nine
representative slices (3 along each axis) per run to identify gross artifacts,
such as missing brain tissue, blurring, or biologically implausible
segmentation. Table~\ref{tab:qc} details the exclusion rates. To ensure
statistical consistency across the cohort, we randomly subsampled the remaining
runs to retain exactly 26 valid realizations per participant.
