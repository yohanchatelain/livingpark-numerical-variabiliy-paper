...
\begin{proof}
    Note that \((n-1)s_x = \sum_{i=1}^{n} (x_i - \overline{x})^2\) and \((n-1)s_{xy} = \sum_{i=1}^{n} (x_i - \overline{x})(y_i - \overline{y})\) then by using the definitions of standardized variables, we have for the first identity:
    \begin{align*}
...
\begin{figure}[t]
    ...
    \caption{Numerical-Population Variability Ratio (\npvr) for cortical surface
        area (top row in each panel) area and cortical volume (bottom row) in HC
